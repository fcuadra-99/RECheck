        ...
        <section hidden={type !== "Assign"} className="bg-white p-6 rounded-lg shadow-md border mb-6">
          ...
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex justify-between items-center">
              ...
              {reviewType === "Exempt" && selectedReviewers.length === 1 && (
                <span className="text-xs text-green-600 flex items-center gap-1">
                  <Check className="h-3 w-3" /> Auto-assigned to you
                </span>
              )}
              {reviewType !== "Exempt" && selectedReviewers.length < requiredReviewerCount && (
                <span className="text-xs text-orange-600">
                  Please select {requiredReviewerCount} reviewers
                </span>
              )}
            </div>
          </div>
