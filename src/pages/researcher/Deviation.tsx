...
const DeviationReportForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only attempt file uploads if there are files
      if (files.length > 0) {
        ...
        // Upload files using our service
        ...
        if (failedUploads.length > 0) {
          ...
          console.error('File upload failures:', errorMessages);
          setErrors(prev => ({ ...prev, files: `Upload failed: ${errorMessages.join(', ')}` }));
          setLoading(false);
          return;
        }
        
        // Get successful upload URLs - filter out empty urls
        ...
        // If we have no valid URLs despite no errors, something is wrong
        if (uploadedUrls.length === 0 && files.length > 0) {
          console.error('No valid URLs returned from file uploads');
          setErrors(prev => ({ ...prev, files: 'No valid URLs returned from uploads' }));
          setLoading(false);
          return;
        }
      } else {
      ...
      console.log('Submitting deviation report...', { uploadedUrls });
      ...
      if (error) {
        ...
        console.log('Submitted report data:', data[0]);
            ...
            <div className="space-y-10">
              ...
              <section>
                ...
                <div className="rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50 p-8 text-center hover:border-blue-400 hover:bg-blue-50 transition-all">
                  ...
                  {errors.files && <ErrorMsg msg={errors.files} />}
                </div>
              </section>
            </div>
