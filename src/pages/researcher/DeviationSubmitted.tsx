...
const RDeviationSubmissions = () => {
    ...
    useEffect(() => {
        ...
        const fetch = async () => {
            try {
                console.log('Fetching deviations for user:', user);
                const email = user?.email;
                if (!email) {
                    console.warn('No email found for user, cannot fetch deviations');
                    setSubmissions([]);
                    setTypeOptions(['All']);
                    setLoading(false);
                    return;
                ...
                // Log user information to help diagnose the issue
                ...
                console.log('Searching for reports with these candidates:', candidates);

                // First try to fetch by user_id which is more reliable
                ...
                if (userError) {
                    console.error('Error fetching by user ID:', userError);
                } else if (userData && userData.length > 0) {
                    console.log('Found reports by user ID:', userData.length);
                    setSubmissions(userData);
                    const uniqueTypes = Array.from(new Set((userData).map(row => row.type).filter(t => t && t !== '')));
                    setTypeOptions(['All', ...uniqueTypes]);
                    setLoading(false);
                    return;
                }
                
                // Fall back to string matching approach if needed
                console.log('No data found by user ID, trying by reported_by text match');
                ...
                if (error) {
                    ...
                    if ((!rows || rows.length === 0)) {
                        ...
                        // Get table structure to debug field names
                        const { error: descError } = await supabase.rpc('describe_table', { table_name: 'deviation_reports' });
                        if (descError) {
                            console.error('Error describing table:', descError);
                        }
                        
                        // Attempt to get ALL records to inspect
                        const { data: allData, error: allErr } = await supabase.from('deviation_reports').select('*').order('report_submission_date', { ascending: false }).limit(20);
                        
                        if (allErr) {
                            console.error('Error fetching all deviation reports for fallback', allErr);
                            rows = [];
                        } else {
                            console.log('All reports (sample):', allData);
                            
                            // Try multiple field names that might contain user info
                            const fields = ['reported_by', 'reported_by_user', 'user_id', 'created_by'];
                            const lowerCandidates = candidates.map(c => c.toLowerCase());
                            
                            rows = (allData || []).filter((r: any) => {
                                // Check user ID exact match first
                                if (r.reported_by_user === user.id) return true;
                                
                                // Fall back to text matching on various fields
                                for (const field of fields) {
                                    if (r[field]) {
                                        const val = r[field].toString().toLowerCase();
                                        if (lowerCandidates.some(c => val.includes(c))) return true;
                                    }
                                }
                            ...
                            if (rows.length > 0) {
                                console.log('Found matching records:', rows);
                            }
                        }
                    }
                                            ...
                                            <tr key={sub.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    {sub.severity && sub.severity !== '' ? (
                                                        <button
                                                            onClick={() => {
                                                                console.log(`Navigating to: /researcher/deviations/feedback/${sub.id}`);
                                                                navigate(`/researcher/deviations/feedback/${sub.id}`);
                                                            }}
                                                            className={`inline-flex items-center gap-2 px-3 py-1 border text-sm font-medium rounded-md ${
